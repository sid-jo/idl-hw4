...
class ASRDataset(Dataset):
    def __init__(
        ...
        subset_size = max(1, min(len(self.fbank_files), subset_size)) if len(self.fbank_files) > 0 else 0
        ...
        for i in tqdm(range(self.length)):
            ...
            if self.partition != "test-clean":
                ...
                transcript_raw = np.load(os.path.join(self.text_dir, self.text_files[i]), allow_pickle=True)
                if isinstance(transcript_raw, np.ndarray):
                    if transcript_raw.shape == ():
    ...
    def collate_fn(self, batch) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if self.partition != "test-clean":
            ...
            transcript_lengths = torch.LongTensor([x.shape[0] for x in batch_shifted]) # B  
