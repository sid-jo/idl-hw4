...
class SequenceGenerator:
    ...
    def generate_greedy(
        ...
        for _ in range(self.max_length - x.size(1)):
            ...
            next_logits = self._apply_repeat_penalty(next_logits, x, penalty=repeat_penalty)
    ...
    def generate_beam(
        ...
        beam_sequences = x.unsqueeze(1)  # (B, 1, L)
        beam_scores = torch.zeros(batch_size, 1, device=x.device)  # (B, 1)
        finished = torch.zeros(batch_size, 1, dtype=torch.bool, device=x.device)  # (B, 1)

        for _ in range(self.max_length - x.size(1)):
            if finished.all():
                break

            B, K, L = beam_sequences.shape
            # Score each beam independently while preserving batch dimension.
            # This keeps compatibility with deterministic test score functions that
            # assume the original batch size.
            logits_per_beam = []
            for k in range(K):
                logits_per_beam.append(self.score_fn(beam_sequences[:, k, :]))  # (B, V)
            next_logits = torch.stack(logits_per_beam, dim=1)  # (B, K, V)

            next_logits = self._apply_repeat_penalty(next_logits, beam_sequences, penalty=repeat_penalty)
            next_logits = next_logits / temperature
            ...
            topk = min(beam_width, K * vocab_size)
            topk_scores, topk_indices = torch.topk(candidate_scores, k=topk, dim=-1)

            next_beam_idx = topk_indices // vocab_size  # (B, topk)
            next_token_idx = topk_indices % vocab_size  # (B, topk)

            gather_idx = next_beam_idx.unsqueeze(-1).expand(-1, -1, L)
            selected_sequences = beam_sequences.gather(1, gather_idx)  # (B, topk, L)
            beam_sequences = torch.cat([selected_sequences, next_token_idx.unsqueeze(-1)], dim=-1)

            beam_scores = topk_scores
            finished = finished.gather(1, next_beam_idx) | (next_token_idx == self.tokenizer.eos_id)
