...
class ASRTrainer(BaseTrainer):
    ...
    def _train_epoch(self, dataloader):
        ...
        for i, batch in enumerate(dataloader):
            ...
            with torch.autocast(device_type=autocast_device, dtype=torch.float16, enabled=autocast_device == "cuda"):
    ...
    def _validate_epoch(self, dataloader):
        ...
        """
        # Use default (greedy) recognition config for validation
        ...
        if len(references) == 0:
            metrics = {'word_dist': float('nan'), 'wer': float('nan'), 'cer': float('nan')}
        else:
